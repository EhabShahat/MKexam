-- Recommended indexes and constraints
 ...
 -- NOTE: Additional performance optimization indexes are defined in:
 -- db/performance_optimizations.sql
 -- These include composite, covering, BRIN, expression, and partial indexes
 -- for improved query performance and reduced egress costs.
...
-- Enhanced Device Tracking Indexes
-- GIN index for querying IPs within device_info JSONB
create index if not exists idx_device_info_all_ips on public.exam_attempts using gin ((device_info->'allIPs'));

-- Index for fingerprint queries (exact match)
create index if not exists idx_device_info_fingerprint on public.exam_attempts ((device_info->>'fingerprint'));

-- Optional: Index for querying local IPs specifically
create index if not exists idx_device_info_local_ips on public.exam_attempts using gin ((device_info->'allIPs'->'local'));

-- Optional: Index for querying public IPs specifically
create index if not exists idx_device_info_public_ips on public.exam_attempts using gin ((device_info->'allIPs'->'public'));
