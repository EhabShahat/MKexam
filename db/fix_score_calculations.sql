-- ============================================================================
-- FIX: Score Calculation Functions
-- ============================================================================
-- This file fixes the homework and quiz score calculations to count
-- non-attempted exams as 0% instead of excluding them from the average.
-- 
-- Formula: (Sum of all attempted exam scores) / (Total number of exams)
-- Example: 2 exams attempted at 100% each, 14 total exams = 200/14 = 14%
-- ============================================================================

-- Drop and recreate the homework/quiz sync function with correct logic
...
BEGIN
  ...
  WITH homework_exams AS (
    -- Count total homework exams
    SELECT COUNT(*) as total_count
    FROM exams
    WHERE exam_type = 'homework' AND status = 'done'
  ),
  quiz_exams AS (
    -- Count total quiz exams
    SELECT COUNT(*) as total_count
    FROM exams
    WHERE exam_type = 'quiz' AND status = 'done'
  ),
  homework_scores AS (
    -- Sum all homework scores for each student
    SELECT 
      s.id as student_id,
  ...
  quiz_scores AS (
    -- Sum all quiz scores for each student
    SELECT 
      s.id as student_id,
      s.code as student_code,
  ...
  SELECT 
    s.id as student_id,
    s.code as student_code,
    -- Divide total score by total exams (non-attempted count as 0)
    CASE 
...
-- Drop and recreate the comprehensive sync function with the same fix
...
BEGIN
  ...
  -- Update homework and quiz scores using the new calculation method
  WITH homework_exams AS (
    SELECT COUNT(*) as total_count
    FROM exams
    WHERE exam_type = 'homework' AND status = 'done'
...
-- Drop and recreate the single student sync function with the same fix
...
BEGIN
  ...
  -- Calculate homework score (total / count)
  ...
  -- Calculate quiz score (total / count)
