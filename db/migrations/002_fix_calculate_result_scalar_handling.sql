-- Migration: Fix calculate_result_for_attempt to handle scalar correct_answers
-- Date: 2026-02-09
-- Issue: The function was failing with "cannot extract elements from a scalar" error
--        when correct_answers was stored as a scalar value instead of an array
-- Fix: Add type checking to handle both array and scalar correct_answers values

-- This migration updates the calculate_result_for_attempt RPC function
...
BEGIN
  ...
  norm AS (
    SELECT a.id, a.question_type, a.correct_answers, a.student_json, a.points, a.auto_grade_on_answer,
           ...
           CASE WHEN a.question_type IN ('multiple_choice','multi_select') THEN
             -- Handle both array and scalar correct_answers
             CASE 
    ...
    SELECT n.id, n.question_type, n.points,
           CASE
             ...
             WHEN n.question_type IN ('true_false','single_choice') THEN
               CASE
                 WHEN n.student_json IS NULL THEN FALSE
                 -- Handle both array and scalar correct_answers for true_false/single_choice
