-- Migration: Fix calculate_result_for_attempt to handle scalar student answers
-- This fixes the "cannot extract elements from a scalar" error when student answers
-- are stored as strings instead of arrays for multiple_choice questions
             ...
             WHEN n.question_type IN ('true_false','single_choice') THEN
               CASE
                 ...
                 WHEN jsonb_typeof(n.correct_answers) = 'string' THEN
                   (n.student_json::text = n.correct_answers::text)
...
'Calculates exam results with full support for mixed data types: handles boolean/string/array for both correct_answers and student answers. Robust against data format inconsistencies.';
