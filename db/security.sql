...
DO $do$ BEGIN
  ...
  -- Also create a separate admin select policy to avoid conflicts
  IF EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='exam_attempts' AND policyname='exam_attempts_admin_select') THEN
    EXECUTE 'DROP POLICY exam_attempts_admin_select ON public.exam_attempts';
  END IF;
  EXECUTE 'CREATE POLICY exam_attempts_admin_select ON public.exam_attempts FOR SELECT USING (public.is_admin())';
END $do$;
...
DO $do$ BEGIN
  ...
  END IF;
  -- Fixed: Removed circular reference to exam_results to prevent infinite recursion
  -- Now only checks if the exam is published (direct parent relationship)
