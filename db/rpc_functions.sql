...
-- save_attempt_v2(uuid,jsonb,jsonb,int,boolean) -> table(new_version int)
-- Enhanced version with answer delta support for incremental updates
-- Maintains backward compatibility with full answer updates
CREATE OR REPLACE FUNCTION public.save_attempt_v2(
  p_attempt_id uuid,
  p_answers jsonb,
  p_auto_save_data jsonb,
  p_expected_version integer,
  p_is_delta boolean DEFAULT false
)
 RETURNS TABLE(new_version integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  select * into v_row from public.exam_attempts where id=p_attempt_id for update;
  if not found then raise exception 'attempt_not_found'; end if;
  if v_row.submitted_at is not null or v_row.completion_status='submitted' then 
  ...
  -- Handle delta vs full update
  if p_is_delta and p_answers is not null then
    -- Delta mode: merge changes with existing answers
    v_current_answers := coalesce(v_row.answers, '{}'::jsonb);
    v_final_answers := v_current_answers;
    
    -- Apply changed answers
    if p_answers ? 'changed' then
      v_final_answers := v_final_answers || (p_answers->'changed');
    end if;
    
    -- Remove deleted answers
    if p_answers ? 'removed' and jsonb_typeof(p_answers->'removed') = 'array' then
      -- Remove each key in the removed array
      declare
        v_key text;
      begin
        for v_key in select jsonb_array_elements_text(p_answers->'removed')
        loop
          v_final_answers := v_final_answers - v_key;
        end loop;
      end;
    end if;
  else
    -- Full update mode (backward compatible)
    v_final_answers := coalesce(p_answers, '{}'::jsonb);
  end if;

  -- Update the attempt with final answers
  update public.exam_attempts
  set answers = v_final_answers,
      auto_save_data = coalesce(p_auto_save_data, '{}'::jsonb),
      version = v_row.version + 1,
      updated_at = now()
  where id = p_attempt_id;
...
-- start_attempt_v3 with device_info optimization
-- Enhanced version that accepts device_info and optimizes it before storage
CREATE OR REPLACE FUNCTION public.start_attempt_v3(
  p_exam_id uuid,
  p_code text,
  p_student_name text,
  p_ip inet,
...
BEGIN
  ...
  -- Optimize device_info if provided
  v_optimized_device_info := p_device_info;
  if p_device_info is not null then
    -- Deduplicate IP arrays in allIPs structure
    if p_device_info ? 'allIPs' then
      declare
        v_all_ips jsonb;
        v_local_ips jsonb;
        v_public_ips jsonb;
      begin
        v_all_ips := p_device_info->'allIPs';
        
        -- Deduplicate local IPs
        if v_all_ips ? 'local' and jsonb_typeof(v_all_ips->'local') = 'array' then
          select jsonb_agg(distinct value) into v_local_ips
          from jsonb_array_elements(v_all_ips->'local');
          v_all_ips := jsonb_set(v_all_ips, '{local}', coalesce(v_local_ips, '[]'::jsonb));
        end if;
        
        -- Deduplicate public IPs
        if v_all_ips ? 'public' and jsonb_typeof(v_all_ips->'public') = 'array' then
          select jsonb_agg(distinct value) into v_public_ips
          from jsonb_array_elements(v_all_ips->'public');
          v_all_ips := jsonb_set(v_all_ips, '{public}', coalesce(v_public_ips, '[]'::jsonb));
        end if;
        
        v_optimized_device_info := jsonb_set(v_optimized_device_info, '{allIPs}', v_all_ips);
      end;
    end if;
  end if;
  ...
  insert into public.exam_attempts(id, exam_id, student_id, ip_address, student_name, answers, auto_save_data, completion_status, version, device_info)
  values(
    v_attempt_id,
    p_exam_id,
    case when v_exam.access_type='code_based' then v_student.id else null end,
...
-- Grant execute permissions for start_attempt_v3
...
DECLARE
  ...
  v_batch_limit integer := 100; -- Process 100 attempts at a time (optimized batch size)
...
BEGIN
  ...
  -- Collect all attempt IDs for this exam
  SELECT ARRAY(SELECT id FROM public.exam_attempts WHERE exam_id = p_exam_id ORDER BY started_at)
  INTO v_attempt_ids;
  ...
  v_total := array_length(v_attempt_ids, 1);

  -- Process in batches of 100
  WHILE v_offset < v_total LOOP
    -- Extract batch
    v_batch := v_attempt_ids[v_offset + 1 : LEAST(v_offset + v_batch_size, v_total)];
    
    -- For each attempt in the batch, save old scores and recalculate
    FOR v_result IN 
      SELECT 
        aid AS attempt_id,
        r.total_questions,
        ...
        r.manual_points,
        r.max_points,
        r.final_score_percentage
      FROM unnest(v_batch) AS aid
      CROSS JOIN LATERAL (
        SELECT 
          er.score_percentage AS old_score,
          COALESCE(er.final_score_percentage, er.score_percentage) AS old_final
        FROM public.exam_results er
        WHERE er.attempt_id = aid
      ) old_scores
      CROSS JOIN LATERAL public.calculate_result_for_attempt(aid) AS r
    LOOP
      -- Get old scores for history
      SELECT er.score_percentage, COALESCE(er.final_score_percentage, er.score_percentage)
      INTO v_old_score, v_old_final
      FROM public.exam_results er
      WHERE er.attempt_id = v_result.attempt_id;
      ...
      VALUES (
        v_result.attempt_id, v_old_score, v_result.score_percentage,
        v_old_final, v_result.final_score_percentage, '{}'::jsonb, now()
      );
...
-- Batch calculate results for multiple attempts (admin only)
-- Processes attempts in batches of 100 to reduce overhead
...
BEGIN
  ...
  v_total := array_length(p_attempt_ids, 1);

  -- Process in batches of 100
  WHILE v_offset < v_total LOOP
    -- Extract batch
    v_batch := p_attempt_ids[v_offset + 1 : LEAST(v_offset + v_batch_size, v_total)];
    
    -- Calculate results for each attempt in the batch
    FOR v_result IN 
      SELECT 
        aid AS attempt_id,
        r.total_questions,
...
-- get_attempt_state_v2(uuid, jsonb) -> jsonb
-- Enhanced version with field selection support and compression
...
DECLARE
  ...
  v_stages jsonb;
  v_stage_progress jsonb;
  v_include_fields text[];
  v_exclude_fields text[];
  v_include_exam boolean := true;
  v_include_questions boolean := true;
  v_include_answers boolean := true;
  v_include_auto_save boolean := true;
  v_include_stages boolean := true;
  v_include_stage_progress boolean := true;
BEGIN
  -- Fetch attempt data
  SELECT * INTO v_row FROM public.exam_attempts WHERE id = p_attempt_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'attempt_not_found'; END IF;
  ...
  IF p_fields IS NOT NULL THEN
    v_include_fields := ARRAY(SELECT jsonb_array_elements_text(p_fields->'include'));
    v_exclude_fields := ARRAY(SELECT jsonb_array_elements_text(p_fields->'exclude'));
    
    -- If include list is specified, only include those fields
    IF v_include_fields IS NOT NULL AND array_length(v_include_fields, 1) > 0 THEN
      v_include_exam := 'exam' = ANY(v_include_fields);
      v_include_questions := 'questions' = ANY(v_include_fields);
      v_include_answers := 'answers' = ANY(v_include_fields);
      v_include_auto_save := 'auto_save_data' = ANY(v_include_fields);
      v_include_stages := 'stages' = ANY(v_include_fields);
      v_include_stage_progress := 'stage_progress' = ANY(v_include_fields);
    -- If exclude list is specified, exclude those fields
    ELSIF v_exclude_fields IS NOT NULL AND array_length(v_exclude_fields, 1) > 0 THEN
      v_include_exam := NOT ('exam' = ANY(v_exclude_fields));
      v_include_questions := NOT ('questions' = ANY(v_exclude_fields));
      v_include_answers := NOT ('answers' = ANY(v_exclude_fields));
      v_include_auto_save := NOT ('auto_save_data' = ANY(v_exclude_fields));
      v_include_stages := NOT ('stages' = ANY(v_exclude_fields));
      v_include_stage_progress := NOT ('stage_progress' = ANY(v_exclude_fields));
    END IF;
  END IF;

  -- Build base response with always-included fields
  v_json := jsonb_build_object(
    'attemptId', v_row.id,
    'version', v_row.version,
    'started_at', v_row.started_at,
  ...
  -- Add exam data if requested
  IF v_include_exam THEN
    v_json := v_json || jsonb_build_object(
      'exam', jsonb_build_object(
        'id', v_exam.id,
        'title', v_exam.title,
  ...
  IF v_include_answers THEN
    v_json := v_json || jsonb_build_object('answers', v_row.answers);
  END IF;

  -- Add auto_save_data if requested
  IF v_include_auto_save THEN
    v_json := v_json || jsonb_build_object('auto_save_data', v_row.auto_save_data);
  END IF;
  ...
  IF v_include_stages THEN
    ...
    v_json := v_json || jsonb_build_object('stages', v_stages);
  END IF;

  -- Add stage_progress if requested
  IF v_include_stage_progress THEN
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        'id', sp.id,
        'attempt_id', sp.attempt_id,
    ...
    v_json := v_json || jsonb_build_object('stage_progress', v_stage_progress);
  END IF;
...
-- Grant execute permissions
GRANT EXECUTE ON FUNCTION public.get_attempt_state_v2(uuid, jsonb) TO anon, authenticated;


-- admin_list_attempts_v2(uuid, integer, integer, jsonb) -> table
-- Enhanced version with pagination and field selection support
-- Uses materialized view for performance with fallback to original tables
-- Requirements: 1.3, 1.2, 10.1, 2.1
...
DECLARE
  v_include_fields text[];
  v_exclude_fields text[];
  v_use_materialized_view boolean := true;
...
BEGIN
  ...
  IF p_limit > 1000 THEN
    p_limit := 1000; -- Maximum limit to prevent excessive data transfer
  END IF;
  ...
  -- Use materialized view if it exists, otherwise fallback to original tables
  IF v_view_exists THEN
    ...
    SELECT 
      ...
      mv.code,
      mv.score_percentage,
      mv.final_score_percentage,
      mv.manual_total_count,
      mv.manual_graded_count,
      mv.manual_pending_count
    FROM public.admin_attempts_summary mv
    WHERE mv.exam_id = p_exam_id
    ORDER BY mv.started_at DESC NULLS FIRST
    LIMIT p_limit
    OFFSET p_offset;
  ELSE
    -- Fallback to original query with JOINs if materialized view doesn't exist
    RETURN QUERY
    SELECT a.id,
           a.exam_id,
           a.started_at,
...
-- Grant execute permissions
GRANT EXECUTE ON FUNCTION public.admin_list_attempts_v2(uuid, integer, integer, jsonb) TO service_role;
