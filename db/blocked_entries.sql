...
DO $do$ BEGIN
  IF EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'blocked_entries' AND policyname = 'Admin can manage blocked entries'
  ) THEN
    EXECUTE 'DROP POLICY "Admin can manage blocked entries" ON public.blocked_entries';
  END IF;
  EXECUTE 'CREATE POLICY "Admin can manage blocked entries" ON public.blocked_entries FOR ALL USING (public.is_admin()) WITH CHECK (public.is_admin())';
END $do$;
