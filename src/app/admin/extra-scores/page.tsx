...
import { loadXLSX } from "@/lib/lazyComponents";
...
export default function ExtraScoresAdminPage() {
  ...
  // Sync engine hook for automatic and manual sync operations
  const { 
    syncStatus, 
    timestamps, 
    syncAll, 
  ...
  useEffect(() => {
    if (syncStatus.lastResult) {
      if (syncStatus.lastResult.success) {
        toast.success({ 
          title: "Auto-sync completed", 
          message: syncStatus.lastResult.message 
        });
      } else {
        toast.error({ 
          title: "Auto-sync failed", 
          message: syncStatus.lastResult.error || "Unknown error" 
        });
      }
    }
  }, [syncStatus.lastResult, toast]);
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              ) : timestamps?.lastFullSync ? (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-green-50 border border-green-200 rounded-full">
                  <svg className="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-sm font-medium text-green-700">
                    Synced {new Date(timestamps.lastFullSync).toLocaleTimeString()}
                  </span>
                </div>
              ) : (
