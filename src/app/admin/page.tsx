...
export default function AdminHomePage() {
  ...
  // Home buttons visibility (controls Exams/Results/Register/ID buttons on main page)
  const { data: homeButtons, isLoading: loadingHomeButtons } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ exams: boolean | null; results: boolean | null; register: boolean | null; id: boolean | null }>;
    },
  });
  ...
  const updateHomeButtonsMutation = useMutation({
    mutationFn: async (payload: Partial<{ exams: boolean | null; results: boolean | null; register: boolean | null; id: boolean | null }>) => {
      const res = await authFetch("/api/admin/system?action=home-buttons", {
        method: "PATCH",
  ...
  const idVisible =
    homeButtons?.id !== null && homeButtons?.id !== undefined
      ? !!homeButtons?.id
      : false; // Default to false (hidden)
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl shadow-sm border border-blue-100">
        <div>
          ...
          <p className="text-gray-600 mt-2 ml-9">
            Manage exams, view results, and control system access
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/admin/results" className="btn btn-secondary btn-sm hover:scale-105 transition-transform">
      ...
      <ModernCard className={`border-l-4 ${systemMode === 'disabled' ? 'border-l-red-500 bg-red-50' : systemMode === 'results' ? 'border-l-blue-500 bg-blue-50' : 'border-l-green-500 bg-green-50'} shadow-md transition-all duration-300 transform hover:scale-[1.01]`}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {systemMode === 'disabled'
                  ...
                  : `Exams: ${examsVisible ? 'Visible' : 'Hidden'} • Results: ${resultsVisible ? 'Visible' : 'Hidden'} • Register: ${registerVisible ? 'Visible' : 'Hidden'} • ID Card: ${idVisible ? 'Visible' : 'Hidden'}`}
              </p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 w-full">
            ...
            {/* ID Card button visibility toggle */}
            <div className="flex items-center justify-between sm:justify-start gap-3 w-full sm:w-auto">
              <span className="text-sm font-medium text-gray-800">ID Card</span>
              <button
                type="button"
                aria-pressed={idVisible}
                onClick={() => updateHomeButtonsMutation.mutate({ id: !idVisible })}
                disabled={loadingHomeButtons || updateHomeButtonsMutation.isPending}
                className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors ${
                  idVisible ? 'bg-emerald-500' : 'bg-gray-300'
                } ${(loadingHomeButtons || updateHomeButtonsMutation.isPending) ? 'opacity-60 cursor-not-allowed' : 'hover:opacity-90'}`}
                title={idVisible ? 'Hide ID Card button on home page' : 'Show ID Card button on home page'}
              >
                <span
                  className={`inline-block h-6 w-6 transform rounded-full bg-white shadow transition-transform ${
                    idVisible ? 'translate-x-7' : 'translate-x-1'
                  }`}
                />
              </button>
            </div>
