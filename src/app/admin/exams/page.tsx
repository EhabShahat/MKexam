...
import ModernTable from "@/components/admin/ModernTable/index";
...
import { useOptimisticListItemUpdate, useOptimisticListItemRemove } from "@/hooks/useOptimisticMutation";
...
export default function AdminExamsPage() {
  ...
  const deleteMutation = useOptimisticListItemRemove<Exam, string>({
    mutationFn: async (examId: string) => {
    ...
    queryKey: ["admin", "exams", searchQuery],
    itemMatcher: (item, examId) => item.id === examId,
    onSuccess: () => {
  ...
  // Publish exam mutation with optimistic update
  const publishMutation = useOptimisticListItemUpdate<Exam, { id: string; is_manually_published: boolean }>({
    mutationFn: async ({ id }: { id: string; is_manually_published: boolean }) => {
  ...
  // Unpublish exam mutation with optimistic update
  const unpublishMutation = useOptimisticListItemUpdate<Exam, { id: string; is_manually_published: boolean }>({
    mutationFn: async ({ id }: { id: string; is_manually_published: boolean }) => {
      const res = await authFetch(`/api/admin/exams/${id}`, {
        method: "PATCH",
  ...
  // Archive exam mutation with optimistic update
  const archiveMutation = useOptimisticListItemUpdate<Exam, { id: string; is_archived: boolean }>({
    mutationFn: async ({ id }: { id: string; is_archived: boolean }) => {
      const res = await authFetch(`/api/admin/exams/${id}`, {
        method: "PATCH",
    ...
    itemMatcher: (item, variables) => item.id === variables.id,
    onSuccess: () => {
      toast.success({ title: "Exam Archived", message: "Exam is no longer accessible to students" });
    },
    onError: (error: Error) => {
  ...
  // Unarchive exam mutation with optimistic update
  const unarchiveMutation = useOptimisticListItemUpdate<Exam, { id: string; is_archived: boolean }>({
    mutationFn: async ({ id }: { id: string; is_archived: boolean }) => {
      const res = await authFetch(`/api/admin/exams/${id}`, {
        method: "PATCH",
    ...
    switch (column.key) {
      ...
      case "status_actions":
        ...
        if (exam.is_archived) {
          smartAction = {
            label: "Unarchive",
            onClick: () => unarchiveMutation.mutate({ id: exam.id, is_archived: false }),
          ...
          smartAction = {
            label: "Unpublish",
            onClick: () => unpublishMutation.mutate({ id: exam.id, is_manually_published: false }),
          ...
          smartAction = {
            label: "Archive",
            onClick: () => archiveMutation.mutate({ id: exam.id, is_archived: true }),
          ...
          smartAction = {
            label: "Publish",
            onClick: () => publishMutation.mutate({ id: exam.id, is_manually_published: true }),
