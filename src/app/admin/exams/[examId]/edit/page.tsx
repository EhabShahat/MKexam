...
export default function AdminEditExamPage() {
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'stages'>('basic');
  ...
  const publishMutation = useOptimisticMutation({
    ...
    queryKey: ["admin", "exam", examId],
    updateCache: (oldData, _variables) => {
      if (!oldData) return oldData;
      return { ...oldData, status: 'published' };
    },
    onSuccess: (item) => {
  ...
  const saveMutation = useOptimisticMutation({
    mutationFn: async () => {
      ...
      if (isNewExam) {
        ...
        // Update existing exam - exclude stages as they're saved separately
        const { stages: _, ...examData } = exam;
        const res = await authFetch(`/api/admin/exams/${examId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        ...
        // Save stages if there are any changes
        if (stages.length > 0 || data?.stages?.length > 0) {
          const stagesRes = await authFetch(`/api/admin/exams/${examId}/stages`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(stages),
          });
          const stagesResult = await stagesRes.json();
          if (!stagesRes.ok) throw new Error(stagesResult?.error || "Failed to save stages");
          
          // Update the result with saved stages
          result.item.stages = stagesResult.stages;
        }
    ...
    queryKey: ["admin", "exam", examId],
    updateCache: (oldData, _variables) => {
      // For new exams, don't update cache (no existing data)
      if (isNewExam) return oldData;
      // For existing exams, optimistically update with local changes
      return exam;
    },
    onSuccess: (item) => {
      if (isNewExam) {
        ...
        setLocalChanges(item);
        // Update stages if they were saved
        if (item.stages) {
          setStages(item.stages);
        }
        queryClient.invalidateQueries({ queryKey: ["admin", "exams"] });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto p-6">
        ...
        {/* Questions Section - Only show for existing exams on basic tab */}
        {!isNewExam && activeTab === 'basic' && (
          <QuestionsSection
            examId={examId}
