...
import { LazyBarChart, loadXLSX, loadJsPDF } from "@/lib/lazyComponents";
import { useTheme } from "@/hooks/useTheme";
import { getThemedChartOptions, getChartColors } from "@/lib/chartTheme";
...
function toArrayStrSorted(v: unknown) {
  const arr = Array.isArray(v) ? v : v == null ? [] : [v];
  return arr.map((x) => normStr(x)).sort();
}
...
function isCorrect(question: any, answer: unknown): boolean | null {
  ...
  if (t === "true_false") {
    return boolStr(answer) === boolStr(correct);
  }
  if (t === "single_choice") {
    const corr = Array.isArray(correct) ? (correct.length ? correct[0] : null) : correct;
    return normStr(answer) === normStr(corr);
  }
  if (t === "multiple_choice" || t === "multi_select") {
    const aa = toArrayStrSorted(answer);
    const bb = toArrayStrSorted(correct);
    if (aa.length !== bb.length) return false;
    for (let i = 0; i < aa.length; i++) if (aa[i] !== bb[i]) return false;
    return true;
  }
...
export default function AnalysisPage() {
  ...
  const analysisQ = useQuery({
    ...
    queryFn: async () => {
      ...
      const autoRows = rows.filter((r) => r.autoGradable);
      const agg = autoRows.reduce((acc, r) => { acc.c += r.correct; acc.t += r.total; return acc; }, { c: 0, t: 0 });
      const summary = {
        attempts_considered: ids.length,
        questions_considered: rows.length,
        avg_correct_rate: agg.t ? Math.round((agg.c / agg.t) * 100) : 0,
      };
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Link href={`/admin/results/analysis/${examId}/stages`} className="btn btn-sm">
              ...
              <LazyBarChart
                data={{
                  ...
                  datasets: [{
                    ...
                    backgroundColor: getChartColors(theme).primary,
                  }],
                }}
                options={getThemedChartOptions(theme, {
                  plugins: { 
                  ...
                  <LazyBarChart
                    data={{
                      ...
                      datasets: [{ 
                        label: "Attempts", 
                        data: bins, 
                        backgroundColor: getChartColors(theme).secondary 
                      }],
                    }}
                    options={getThemedChartOptions(theme, {
