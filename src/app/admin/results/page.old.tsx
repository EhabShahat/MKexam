...
import VirtualModernTable from "@/components/admin/VirtualModernTable";
...
export default function AdminResultsIndex() {
  ...
  const { 
    ...
    autoSyncOnMount: false, // Disabled to prevent RLS errors
  ...
  // Apply status filter to visible exams using proper scheduling logic
  const filteredVisibleExams = useMemo(() => {
    let filtered = visibleExams;
    
    // Apply status filter
    if (statusFilter === 'published') {
      // Use getExamStatus to properly check if exam is live/published
      filtered = filtered.filter((e) => {
        const examStatus = getExamStatus(e);
        return examStatus.isLive; // This checks both Manual and Auto modes correctly
      });
    } else if (statusFilter === 'completed') {
      filtered = filtered.filter((e) => {
        // Exclude archived exams from completed view (check both fields)
        if (e.is_archived || e.status === 'archived') return false;
        const examStatus = getExamStatus(e);
        return examStatus.status === 'Done';
      });
    }
    
    // Apply search filter
    if (examSearch.trim()) {
      const searchTerm = examSearch.toLowerCase().trim();
      filtered = filtered.filter((e) => 
    ...
    return filtered;
  }, [visibleExams, statusFilter, examSearch]);
  ...
  const allAttemptsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('[All Exams] Bulk API response:', {
        totalAttempts: json.totalAttempts,
        examCount: json.examCount,
        sampleAttempt: json.allAttempts?.[0]
      });
      
      const resultsByExam: Record<string, any[]> = json.attemptsByExam || {};
  ...
  const renderCell = (attempt: Attempt, column: any) => {
    switch (column.key) {
      ...
      case "time":
        ...
        const startTime = attempt.started_at ? new Date(attempt.started_at) : null;
          ...
          <div className="text-sm">
            <div className="text-gray-900">
              ...
              {durationMinutes !== null && (
                <span className="ml-1 text-gray-500">({durationMinutes}m)</span>
              )}
            </div>
          </div>
        );
      ...
      case "ip":
        // Count how many times this IP appears in the current exam
        const currentIP = attempt.ip_address;
        const ipCount = currentIP 
          ? sortedAttempts.filter(a => a.ip_address === currentIP).length 
          : 0;
        ...
        return (
          <div className="text-sm">
            <div className="text-gray-900">
              {currentIP || "-"}
              {ipCount > 1 && (
                <span className="ml-1 text-gray-500">({ipCount})</span>
              )}
            </div>
          </div>
        );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl border border-gray-200/50 shadow-lg p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
            ...
            <Link href="/admin/results/matrix">
              <button
                type="button"
                className="group relative p-4 rounded-xl border-2 transition-all duration-200 bg-white text-gray-800 border-gray-200 hover:border-purple-300 hover:shadow-md hover:scale-102 w-full"
              >
                <div className="flex items-center justify-center mb-2">
            ...
            <div className="overflow-x-auto touch-pan-x">
              <VirtualModernTable
                ...
                onRowClick={(attempt) => {
                  ...
                  window.location.href = `/admin/results/${attempt.id}`;
                }}
