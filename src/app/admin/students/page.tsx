...
import { useOptimisticMutation, useOptimisticListItemUpdate, useOptimisticListItemRemove } from "@/hooks/useOptimisticMutation";
...
export default function GlobalStudentsPage() {
  ...
  // Note: For datasets with 500+ students, consider using VirtualizedGrid component
  // from @/components/VirtualizedList/VirtualizedGrid for better performance
  ...
  const saveRow = useMutation<Student, Error, { id: string; payload: any; photoFile?: File | null; nationalIdPhotoFile?: File | null }>({
    ...
    onSuccess: (_item, vars) => {
      ...
      setActionError(null);
      qc.invalidateQueries({ queryKey: ["admin", "students", "global"] });
      toast.success({ title: "Student Updated", message: "Changes saved successfully" });
    },
    onError: (error: Error) => {
  ...
  const deleteRow = useOptimisticListItemRemove<Student, string>({
    ...
    queryKey: ["admin", "students", "global"],
    itemMatcher: (item, id) => item.student_id === id,
    onSuccess: () => {
      setActionError(null);
      toast.success({ title: "Student Deleted", message: "Student and all related data removed" });
    },
    onError: (error: Error) => {
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-success/10 hover:bg-success/20 border border-success/30 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isExporting ? (
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Add Student */}
        <div className="bg-card/90 backdrop-blur-sm rounded-2xl border border-border/50 shadow-lg overflow-visible">
          <div 
            className="px-6 py-4 bg-success/10 border-b border-success/20 cursor-pointer hover:bg-success/15 transition-colors"
                ...
                {newPhotoPreview ? (
                  <div className="relative w-32 h-32">
                    ...
                    <button
                      ...
                      className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center text-sm transition-colors z-10"
                    >
                      ×
                    </button>
                  </div>
        ...
        {/* Students Table */}
        <div className="bg-card/90 backdrop-blur-sm rounded-2xl border border-border/50 shadow-lg overflow-hidden">
          ...
          <div className="p-4">
            {paginatedStudents.length === 0 && filtered.length === 0 ? (
              ...
              // Use VirtualGrid for lists > 50 items, otherwise use standard grid
              <VirtualGrid
                ...
                renderItem={(s) => {
                  ...
                  return (
                    <div
                      key={studentId}
                      className="group bg-card rounded-2xl border border-border hover:border-primary hover:shadow-xl transition-all duration-300 overflow-hidden"
                    >
                      {/* Avatar Section */}
                      <div className="relative bg-muted/30 px-6 pt-6 pb-4">
                        ...
                        <div className="flex justify-center">
                          {s.photo_url ? (
                            <button
                              type="button"
                              className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg hover:scale-110 hover:border-blue-400 transition-all duration-300 cursor-pointer relative"
                            ...
                            <div className="w-24 h-24 rounded-full bg-primary/20 border-4 border-card shadow-lg flex items-center justify-center">
...
function FullScreenImageViewer({ 
  ...
  return (
    <div className="fixed inset-0 bg-black z-[60] flex items-center justify-center">
      {/* Header */}
      <div className="absolute top-0 left-0 right-0 bg-black/80 z-10 p-4 sm:p-6">
        <div className="flex items-center justify-between">
      ...
      <div className="w-full h-full flex items-center justify-center p-4 sm:p-8 relative">
        <div className="relative max-w-full max-h-full" style={{ width: '90vw', height: '80vh' }}>
          <Image
            src={url}
            alt={title}
            fill
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-5xl h-[90vh] overflow-hidden shadow-2xl flex flex-col md:flex-row">
        ...
        {/* LEFT SIDEBAR */}
        <div className="w-full md:w-80 bg-muted/30 border-r border-border flex flex-col overflow-y-auto">
            ...
            <div className="relative">
              {photoPreview ? (
                <button
                  ...
                  className="w-32 h-32 rounded-2xl overflow-hidden border-4 border-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 relative"
                >
                  <Image
                    src={photoPreview}
                ...
                <div className="w-32 h-32 rounded-2xl bg-muted flex items-center justify-center border-4 border-card shadow-lg">
            ...
            <div className="relative">
              {nationalIdPhotoPreview ? (
                <button
                  ...
                  className="w-full h-32 rounded-xl overflow-hidden border-2 border-gray-200 shadow hover:shadow-lg transition-all relative"
                >
                  <Image
