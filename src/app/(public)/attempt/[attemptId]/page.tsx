// Force dynamic rendering for attempt pages (Requirement 3.1)
// Attempt pages must always be server-rendered on demand, never statically generated
...
function isQuestionAnswered(question: Question, answer: AnswerValue): boolean {
  ...
  switch (question.question_type) {
    ...
    case "true_false":
      // Handle both boolean and string representations for compatibility
      if (typeof answer === "boolean") return true;
      if (typeof answer === "string") {
        const normalized = answer.toLowerCase().trim();
        return normalized === "true" || normalized === "false" || normalized === "صحيح" || normalized === "خطأ";
      }
      return false;
    case "multi_select":
...
export default function AttemptPage() {
  ...
  // Activity batching to reduce function calls to /activity
  ...
  const [currentStageIndex, setCurrentStageIndex] = useState(0); // Track current stage
  ...
  // Create debounced save function (Requirement 4.5: 3 second debounce delay)
  ...
  const answered = useMemo(() => {
    const count = countAnswered(answers, state?.questions || []);
    
    // Debug: Log unanswered questions to help identify the issue
    if (state?.questions && count < state.questions.length) {
      const unanswered = state.questions.filter(q => !isQuestionAnswered(q, answers[q.id]));
      console.log('Unanswered questions:', unanswered.map(q => ({
        id: q.id,
        type: q.question_type,
        text: q.question_text?.substring(0, 50),
        answer: answers[q.id],
        answerType: typeof answers[q.id]
      })));
    }
  ...
  const autoSaveIntervalSec = useMemo(() => {
    ...
    return Number(s?.auto_save_interval ?? 30);
  }, [state?.exam?.settings]);
  ...
  // Check if this is a staged exam
  const isStaged = useMemo(() => {
    return state?.stages && state.stages.length > 0;
  }, [state?.stages]);
  ...
  useEffect(() => {
    ...
    saveTimer.current = setInterval(() => {
      void saveNow();
    }, Math.max(15, autoSaveIntervalSec) * 1000);
    return () => {
      if (saveTimer.current) clearInterval(saveTimer.current);
    };
  ...
  // Initialize debounced save function (Requirement 4.5: 3 second debounce)
  useEffect(() => {
    debouncedSaveRef.current = debounce(() => {
      void saveNow();
    }, 3000); // 3 second debounce delay as per requirement
    
    return () => {
      // Cleanup: cancel any pending debounced saves on unmount
      debouncedSaveRef.current?.cancel();
    };
  }, []);

  function scheduleSave() {
    // Use debounced save function (Requirement 4.5)
  ...
  async function saveNow() {
    ...
    const payloadStr = JSON.stringify(payloadAnswers);
    if (payloadStr === lastSavedAnswersRef.current) {
      return;
    }
    ...
    try {
      ...
      while (retry < 2) {
        // snapshot answers for this save attempt
        const payloadAnswers = answersRef.current;
        const expectedVersion = versionRef.current;
        const { data, error } = await supabaseClient.rpc("save_attempt", {
          p_attempt_id: attemptId,
          p_answers: payloadAnswers ?? {},
          p_auto_save_data: { progress: { answered, total } },
          p_expected_version: expectedVersion ?? 1,
        });
        if (error) {
          if ((error.message || "").includes("version_mismatch")) {
            const latest = await supabaseClient.rpc("get_attempt_state", { p_attempt_id: attemptId });
            const latestVersion = (latest.data as any)?.version as number | undefined;
            if (latestVersion) {
        ...
        if (row?.new_version) {
          setVersion(row.new_version);
          versionRef.current = row.new_version;
        }
        // Remember last saved answers snapshot to prevent redundant saves
        try { lastSavedAnswersRef.current = JSON.stringify(payloadAnswers); } catch {}
        setSaveStatus("saved");
  ...
  async function onSubmit() {
    if (!state || !attemptId) {
      console.error("onSubmit: Missing state or attemptId", { state: !!state, attemptId });
      return;
    }
    
    // Prevent double submission with more robust checks
    if (submitting || state.completion_status === "submitted") {
      console.log("onSubmit: Submission already in progress or completed", {
        submitting,
    ...
    if (window.location.pathname.includes('/thank-you/')) {
      console.log("onSubmit: Already on thank you page, skipping submission");
      return;
    }
    
    // CRITICAL SAFETY CHECK: Prevent immediate submission
    // MANUAL SUBMISSION (student clicks submit): requires 30 seconds minimum
    // AUTO-SUBMISSION (timer expires): requires 50% of exam duration (handled in Timer component)
    try {
      const startTime = new Date(state.started_at).getTime();
      
      // Validate start time is sane
      if (isNaN(startTime) || startTime < 0) {
        console.error("onSubmit: Invalid start time, refusing submission", {
          started_at: state.started_at,
          startTime
        });
        setError("System error: Invalid exam start time. Please contact support.");
        return;
      }
      ...
      if (timeElapsed < MIN_RUNTIME_MS) {
        console.warn("onSubmit: Exam hasn't run long enough for submission", {
          elapsed: Math.floor(timeElapsed / 1000) + "s",
          required: Math.floor(MIN_RUNTIME_MS / 1000) + "s",
          message: "Please wait at least 30 seconds before submitting"
        });
        setError("Please wait at least 30 seconds before submitting the exam.");
        return;
      }
      
      // Additional sanity check: start time shouldn't be in the future
      if (startTime > currentTime + 60000) {
        console.error("onSubmit: Start time is in the future, refusing submission", {
          started_at: state.started_at,
          currentTime: new Date(currentTime).toISOString()
        });
        setError("System error: Invalid exam timing. Please contact support.");
        return;
      }
      
      console.log("onSubmit: All validation checks passed, starting exam submission", {
        attemptId,
      ...
      console.error("onSubmit: Validation error", validationError);
      setError("System error during submission validation. Please try again.");
      return;
    }
    ...
    try {
      ...
      console.log("onSubmit: Exam submitted successfully, redirecting to thank you page");
      ...
      setTimeout(() => {
        try {
          ...
          console.error("onSubmit: Redirect error", error);
      ...
      console.error("onSubmit: Submission error", e);
  ...
  // Activity logging (batched) to reduce function calls
  async function logActivity(activity: string, details?: any) {
    ...
    try {
      ...
      // Flush if queue is large; otherwise schedule a flush soon
      if (activityQueueRef.current.length >= 8) {
        void flushActivityQueue();
      } else if (!activityFlushTimerRef.current) {
      ...
      console.warn('Failed to queue activity:', error);
    }
  }
  ...
  async function flushActivityQueue() {
    ...
    try {
      ...
      // If it fails, re-queue for next attempt
      activityQueueRef.current.unshift(...batch);
    }
  }

  // Flush activities when navigating away or hiding the page
  useEffect(() => {
    const onBeforeUnload = () => { void flushActivityQueue(); };
    const onVisibility = () => {
      try {
        if (document.visibilityState === 'hidden') void flushActivityQueue();
      } catch {}
    };
    ...
    >
      ...
      <div style={{ 
        ...
        <aside style={{
          ...
          <div style={{ 
            ...
            {!sidebarCollapsed && (
              <h2 style={{ 
                ...
                {isStaged ? t(locale, 'stages') : t(locale, 'questions')}
              </h2>
            )}
            <button
          ...
          {/* Stage Navigation (for staged exams) or Question Navigation (for traditional exams) */}
          {isStaged ? (
            /* Stage Navigation */
            <div style={{ 
              display: 'flex', 
              ...
              {state.stages?.map((stage, idx) => {
                const progress = state.stage_progress?.find(p => p.stage_id === stage.id);
                ...
                return (
                  <div
                    ...
                    style={{
                      width: '100%',
                      minHeight: sidebarCollapsed ? '48px' : '60px',
                      borderRadius: '0.5rem',
                      border: '2px solid',
                      borderColor: isCurrent ? 'var(--primary)' : isCompleted ? '#16a34a' : 'var(--border)',
                      backgroundColor: isCurrent ? 'rgba(59, 130, 246, 0.1)' : isCompleted ? '#f0fdf4' : 'var(--background)',
                      padding: '0.75rem',
                      display: 'flex',
                      ...
                      opacity: isCurrent ? 1 : isCompleted ? 0.8 : 0.6,
                      boxShadow: isCurrent ? '0 0 0 3px rgba(59, 130, 246, 0.2)' : 'none'
                    }}
                    title={`${getStageLabel()} - Stage ${idx + 1}${isCurrent ? ' (Current)' : isCompleted ? ' (Completed)' : ''}`}
                  >
                    <div style={{ 
                      color: isCurrent ? 'var(--primary)' : isCompleted ? '#16a34a' : 'var(--foreground)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center'
                    }}>
        ...
        >
          {/* Conditional rendering: Staged exam vs Traditional exam */}
          {isStaged ? (
            /* Staged Exam Flow */
            <StageContainer
              attemptId={attemptId!}
