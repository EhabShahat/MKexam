...
export async function POST(req: NextRequest, ctx: { params: Promise<{ examId: string }> }) {
  try {
    ...
    // Check if multi-exam mode is enabled
    const settings = await svc.from("app_config").select("enable_multi_exam").single();
    const multiExamEnabled = settings.data?.enable_multi_exam ?? true;

    // Only unpublish other exams if multi-exam mode is disabled
    if (!multiExamEnabled) {
