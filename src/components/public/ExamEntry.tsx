...
import { storeCode } from "@/hooks/useStudentCode";
...
export default function ExamEntry({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      console.log('[ExamEntry] Starting exam access request:', {
        examId,
        accessType: examInfo.access_type,
        hasCode: !!code,
        hasStudentName: !!studentName
      });

      const submitClicks = submitClicksRef.current;
      
      // Log form submission attempt with submit click tracking
      console.log('[ExamEntry] Form submission initiated:', {
        examId,
        accessType: examInfo.access_type,
        submitClickCount: submitClicks.count,
        firstClickAt: submitClicks.timestamps.length > 0 ? submitClicks.timestamps[0] : null,
        lastClickAt: submitClicks.timestamps.length > 0 ? submitClicks.timestamps[submitClicks.timestamps.length - 1] : null,
        totalClicks: submitClicks.timestamps.length,
        timestamp: new Date().toISOString()
      });
      
      const deviceInfo = await collectDeviceInfoWithTimeout({
        count: submitClicks.count,
        firstAt: submitClicks.timestamps.length > 0 ? submitClicks.timestamps[0] : null,
        lastAt: submitClicks.timestamps.length > 0 ? submitClicks.timestamps[submitClicks.timestamps.length - 1] : null,
        timestamps: submitClicks.timestamps
      });

      if (!deviceInfo) {
        console.warn('[ExamEntry] Device info collection returned null, proceeding without device info:', {
          examId,
          willProceedWithoutDeviceInfo: true,
          timestamp: new Date().toISOString()
        });
      } else {
        console.log('[ExamEntry] Device info collected successfully, sending to server:', {
          examId,
      ...
      const data = await res.json();
      if (!res.ok) {
        console.error('[ExamEntry] Exam access request failed:', {
          status: res.status,
          error: data?.error,
      ...
      console.log('[ExamEntry] Exam access granted successfully:', {
        attemptId,
        studentName: studentNameFromResponse,
        hasDeviceInfo: !!deviceInfo
      });

      // Store code on successful validation (only for code-based access)
      if (examInfo.access_type === "code_based" && code) {
        storeCode(code, data.studentId);
      }
