...
export async function resultsGET(request: NextRequest) {
  try {
    ...
    let configMap = new Map<string, { hidden?: boolean; include_in_pass?: boolean; order_index?: number | null }>();
    if (examIds.length > 0) {
    ...
    const items = (data || [])
      ...
      .map((row: any) => {
        ...
        const chosen = examScoreSource === 'final' ? er?.final_score_percentage : er?.score_percentage;
        const score_percentage = chosen === null || chosen === undefined ? null : Number(chosen);
