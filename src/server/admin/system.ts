...
const HOME_BUTTON_KEYS = ["home_button_exams", "home_button_results", "home_button_register", "home_button_id"] as const;
...
async function getHomeButtons(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      id: map.get("home_button_id") === "true" ? true : map.has("home_button_id") ? false : null,
    });
  } catch (e: any) {
    if (e instanceof Response) return e;
    return NextResponse.json({ error: e?.message || "unexpected_error" }, { status: 500 });
...
async function updateHomeButtons(req: NextRequest) {
  try {
    ...
    if (typeof body.id === "boolean") {
      toUpsert.push({ key: "home_button_id", value: body.id ? "true" : "false", updated_at: now });
    }
    ...
    if (body.id === null) {
      const del = await svc.from("app_config").delete().eq("key", "home_button_id");
      if (del.error) return NextResponse.json({ error: del.error.message }, { status: 400 });
    }
