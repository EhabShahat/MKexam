...
function getWeekStartUTC(date: Date): Date {
  ...
  // Make Wednesday the start of the week (Wed-Tue); JS getUTCDay(): 0=Sun, 1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat
...
export async function attendanceHistoryGET(req: NextRequest) {
  try {
    ...
    // Fetch ALL attendance records to determine actual date range
    // Note: Supabase has a default limit, so we need to fetch all pages
    let allRecords: any[] = [];
    let from = 0;
    const pageSize = 1000;
    ...
    if (!records || records.length === 0) {
      return NextResponse.json({ weeks: [], students: [], nonAttendees: { last2Weeks: [], lastMonth: [], threePlusMonths: [] } });
    }

    // Find the earliest and latest session dates
    ...
    // Build week periods from earliest week to latest week
