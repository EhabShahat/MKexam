...
export async function blockedEntriesGET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Failed to fetch blocked entries" }, { status: 500 });
    }
...
export async function blockedEntriesIdDELETE(request: NextRequest, id: string) {
  try {
    ...
    const { data: entry, error: fetchError } = await supabase
      ...
      .single();
    if (fetchError || !entry) {
      return NextResponse.json({ error: "Blocked entry not found" }, { status: 404 });
    }
    ...
    if (deleteError) {
      ...
      return NextResponse.json({ error: "Failed to delete blocked entry" }, { status: 500 });
    }
...
export async function blockedEntriesPOST(request: NextRequest) {
  try {
    ...
    if (!["name", "ip", "mobile"].includes(type)) {
      ...
      return NextResponse.json({ error: "Type must be 'name', 'ip', or 'mobile'" }, { status: 400 });
    }
    ...
    try {
      ...
      if (existingError && (existingError as any).code !== "PGRST116") {
        ...
        return NextResponse.json({ error: "Database error while checking existing entries" }, { status: 500 });
      }

      if (existing) {
        console.log("Entry already exists:", existing);
        return NextResponse.json({ error: "This entry is already blocked" }, { status: 409 });
      }
    } catch (checkError) {
    ...
    try {
      ...
      if (insertResult.error) {
        ...
        return NextResponse.json(
          { error: `Database insert failed: ${(insertResult.error as any).message}` },
          { status: 500 }
        );
      }
      ...
      return NextResponse.json({ error: "Failed to insert blocked entry" }, { status: 500 });
    }
