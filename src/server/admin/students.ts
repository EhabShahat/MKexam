...
import { invalidateStudentCode } from "@/lib/cachedQueries";
...
export async function studentsPOST(req: NextRequest) {
  try {
    ...
    // Invalidate student code cache on insert
    if (finalCode) {
      invalidateStudentCode(finalCode);
    }
...
export async function studentsIdPATCH(req: NextRequest, studentId: string) {
  try {
    ...
    // Invalidate student code cache on update
    if (data && (data as any).code) {
      invalidateStudentCode((data as any).code);
    }
...
export async function studentsBulkPOST(req: NextRequest) {
  try {
    ...
    // Invalidate student code cache for all inserted students
    if (data && Array.isArray(data)) {
      for (const student of data) {
        if ((student as any).code) {
          invalidateStudentCode((student as any).code);
        }
      }
    }
