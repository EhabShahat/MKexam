...
export async function extraScoresSyncExamTagsPOST(req: NextRequest) {
  try {
    ...
    // After syncing custom tags, also sync the standard homework/quiz/attendance scores
    // This ensures all scores are up to date
    await svc.rpc('sync_all_extra_scores');
    ...
    return NextResponse.json({ 
      ...
      message: 'Tag scores and standard scores (homework, quiz, attendance) synced successfully'
    });
  } catch (e: any) {
    if (e instanceof Response) return e;
    return NextResponse.json({ error: e?.message || "unexpected_error" }, { status: 500 });
...
export async function extraScoresSyncAttendancePOST(req: NextRequest) {
  try {
    ...
    const body = await req.json().catch(() => ({}));
    const { fieldKey = "attendance_percentage" } = body;
    
    // 1. Ensure the attendance field exists in extra_score_fields
    const { data: existingField } = await svc
      .from("extra_score_fields")
      .select("id, key")
      .eq("key", fieldKey)
    ...
    // 2. Call the SQL function to sync all attendance data
    const { data: syncResult, error: syncError } = await svc
    ...
    const { count: totalStudents } = await svc
      .from("students")
    ...
    return NextResponse.json({
      success: true,
      field_key: fieldKey,
      updated_students: syncResult?.[0]?.updated_count || totalStudents || 0,
      total_students: totalStudents || 0,
      message: syncResult?.[0]?.message || `Attendance, homework, and quiz data synced successfully`
    });
