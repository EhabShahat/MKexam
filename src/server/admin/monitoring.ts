...
import { fetchMonitoringSummary } from "@/lib/monitoringSummary";

export async function monitoringGET(req: NextRequest) {
  try {
    await requireAdmin(req);
    
    // Check if summary mode is requested (reduces data transfer)
    // Requirements: 7.4
    ...
    // If summary mode, return aggregated statistics only
    if (summaryMode) {
      const summary = await fetchMonitoringSummary(svc);
